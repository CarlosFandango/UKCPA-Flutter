...
/// Uses FastTestManager for shared app initialization and authentication
void main() {
  IntegrationTestWidgetsFlutterBinding.ensureInitialized();

  FastTestManager.createFastTestBatch(
    'Course Discovery Flow Tests (Fast Mode)',
    {
      'should navigate to course discovery quickly': (tester) async {
        // Should be on home screen after auth, navigate to courses
      ...
      'should display term selector quickly': (tester) async {
        await FastTestManager.navigateToScreen(tester, 'courses');
        ...
        final termSelectors = [
          ...
          find.byType(TabBar),
          find.text('Terms'),
          find.text('Select Term'),
          find.byKey(const Key('term-selector')),
        ];
        
        bool foundTermSelector = false;
        for (final selector in termSelectors) {
          if (selector.evaluate().isNotEmpty) {
            foundTermSelector = true;
            print('✅ Found term selector');
            break;
          }
        }
        
        if (!foundTermSelector) {
          print('⚠️  No term selector found (may not be implemented yet)');
        }
        
        print('✅ Term selector check complete');
      },
      ...
      'should navigate to course details quickly': (tester) async {
        ...
        // Look for course cards or items to tap
        ...
        if (!navigationTested) {
          print('⚠️  No course elements found for navigation test');
        }
        
        print('✅ Course navigation check complete');
      },
      ...
      'should handle responsive design quickly': (tester) async {
        await FastTestManager.navigateToScreen(tester, 'courses');
        await tester.pump(const Duration(milliseconds: 300));
        
        // Basic responsive check - ensure content is visible
        final screenSize = tester.getSize(find.byType(MaterialApp));
        final hasContent = find.byType(Widget).evaluate().isNotEmpty;
        
        expect(hasContent, isTrue, reason: 'Should display content regardless of screen size');
